...
parser.add_option("-m","--memeOfqsub",dest="memeOfqsub",default="",help="a little note message")
...
#     jk=myJobTracker(scriptDir=options.scriptDir,NumOfThread=int(options.ThreadsNum))
    callsh_updateDB(scriptDir=options.scriptDir,NumOfThread=int(options.ThreadsNum),logicalpurpose=options.logicalpurpose,memofQsub=options.memeOfqsub)
    print("finish")