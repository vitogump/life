...
#         print(a,b)
    ...
    obsexpcaculator=Caculators.Caculate_S_ObsExp_difference(methodlist,mindeptojudgefix,N_of_targetpop,N_of_refpop,dbvariantstools,options.topleveltablejudgeancestral)
    for vcf,depthconfig in options.targetpopvcffile_withdepth[:]+options.refpopvcffile_withdepth[:]:
        ...
        if re.search(r"indvd[^/]+",vcf)!=None:
            ...
            exit(-1)
    print(vcfnameKEY_depthfilename_titlenameVALUE,obsexpcaculator.MethodToSeqpoplist,obsexpcaculator.species_idx_list,sep="\n")   
    ...
    for i in range(0,totalChroms,20):
        ...
        for row in result:
            ...
            for snp_aligned in target_ref_SNPs[currentchrID]:
                ...
                if not snp:
                    print(currentchrID,curpos,"snp not find,skip")
                    continue
                ...
                for i in range(3,N_of_targetpop+3):
                    if snp_aligned[i]==None:
                        if vcfnameKEY_depthobjVALUE==None:
                            ...
                            sum_depth=0
                            for title in vcfnameKEY_depthfilename_titlenameVALUE[vcfnamelist[i-3]][1:]:
                                idx=vcfnameKEY_depthobjVALUE[vcfnamelist[i-3]].title.index("Depth_for_"+title)
                        ...
                        if methodlist[i-3]=="indvds":
                            AF = float(re.search(r"AF=([\d\.]+);", snp_aligned[i][0]).group(1))
#                             AN = int(re.search(r"AN=(\d+);", snp_aligned[i][0]).group(1))
#                             AC = int(re.search(r"AC=(\d+);", snp_aligned[i][0]).group(1))
#                             refdep_1 = AN - AC
#                             altalleledep_1 = AC
                ...
                for i in range(3+N_of_targetpop,N_of_refpop+N_of_targetpop+3):
                    if snp_aligned[i]==None:
                        if vcfnameKEY_depthobjVALUE==None:
                            ...
                            sum_depth=0
                            for title in vcfnameKEY_depthfilename_titlenameVALUE[vcfnamelist[i-3-N_of_targetpop]][1:]:
                                idx=vcfnameKEY_depthobjVALUE[vcfnamelist[i-3-N_of_targetpop]].title.index("Depth_for_"+title)
                                sum_depth+=int(depth_linelist[idx])
                            if sum_depth>=mindepthtojudefixed:
                                AF=0
                            else:
                        ...
                        if methodlist[i-3-N_of_targetpop]=="indvds":
                            AF = float(re.search(r"AF=([\d\.]+);", snp_aligned[i][0]).group(1))
#                             AN = int(re.search(r"AN=(\d+);", snp_aligned[i][0]).group(1))
#                             AC = int(re.search(r"AC=(\d+);", snp_aligned[i][0]).group(1))
#                             refdep_1 = AN - AC
