...
#        pop.getVcfMap(vcf)
        ...
        for i in range(0,totalChroms,20):
            ...
            for row in result:
                ...
                if currentchrID in pop.VcfIndexMap:
                    ...
                    snpbinmap.SNPsPerBINMap[currentchrID]=win.winValueL
                else:
                    fillNA=[(0,0,0)]
        ...
        #        for chrom in sorted(hscore.HeterozyMap.keys()):
                    for i in range(len(snpbinmap.SNPsPerBINMap[currentchrID])):
                            ...
                            print(currentchrID + "\t" + str(i) + "\t" + str(snpbinmap.SNPsPerBINMap[currentchrID][i][0]) + "\t" + str(snpbinmap.SNPsPerBINMap[currentchrID][i][1]) + "\t" + '%.18f'%(snpsperkb) + "\t" + '%.12f'%(log2snpsperbin), file=outfile)
