...
fsdata_wild=dadi.Spectrum.from_data_dict(dd,pop_ids=['mallard14',"spotbilled13"],polarized=True,projections=[26,26])
...
popt=dadi.Inference.optimize_log(p0,fsdata_wild,func_ex,pts_1,lower_bound=lower_bound,upper_bound=upper_bound,verbose=len(params))
model=func_ex(popt,ns,pts_1)
theta=dadi.Inference.optimal_sfs_scaling(model,fsdata_wild)
print theta
ll_opt=dadi.Inference.ll_multinom(model,fsdata_wild)
print 'title:theta,ll_opt',paramsname
print 'Optimized parameters', repr([theta,ll_opt,popt])
pylab.figure()
dadi.Plotting.plot_single_2d_sfs(fsdata_wild,vmin=1)
pylab.show()
...
pylab.figure()
dadi.Plotting.plot_2d_comp_multinom(model,fsdata_wild,vmin=1)
pylab.show()
