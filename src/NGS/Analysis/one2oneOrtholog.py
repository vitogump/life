...
#"""==================================annotated to use below only=========================================="""
            ...
            if (line.count("Lox|")<2 or line.count("GRCh38|")<2 or line.count("MMUL|")<2 or line.count("PapA|")<=1 or  line.count("Cjac|")<=1 or line.count("CanF|")<=1 or line.count("Pika|")<=1 or line.count("PIG|")<=1 or line.count("turT|")<=1 or line.count("GRCm|")<=1 or line.count("Equ|")<=1) and ((line.count("UMD|")==line.count("GSM|")==line.count("Oar|")==1 ) or line.count("UMD|")==line.count("GSM|")==1 or line.count("Oar|")==line.count("GSM|")==1 ):#or line.count("UMD|")==line.count("GSM|")==1 or line.count("GSM|")==line.count("Oar|")==1 or line.count("Oar|")==line.count("UMD|")==1 #or line.count("UMD|")==1 or line.count("Oar|")==1 or  line.count("GSM|")==1
                ...
                print(line[:50],"is not one-2-one ortholog")
        of.close()
    elif options.steps.strip().lower()=="1t":
        print("transform orthomcl out file to new input format of CalculateKaksusingMusclePaml.py ")
        of=open(options.groupfile+".step"+options.steps.strip().lower()+"out","w")
        for sp in options.targetSpeciesname+options.bgSpeciesname:
            print(sp,end="\t~\t",file=of)
        print("",file=of)
        for line in groupfile:
            linelist=re.split(r"\s+",line.strip());line2print=""
            for  sp in options.targetSpeciesname+options.bgSpeciesname:
                fdsp=False
                for sp_homogene in linelist[1:]:
                    if sp in sp_homogene:fdsp=True;line2print+=(re.sub(r'^'+sp+"\|","",sp_homogene)+",")
                else:
                    if fdsp:
                        line2print=line2print.rstrip(",")+"\t~\t"
                    else:
                        line2print+="NA\t~\t"
            if line2print.replace('NA','').replace("~","").strip():print(line2print.strip().strip("~"),file=of)
        of.close()
        ...
        for fn in os.listdir(options.compliantFasta):
            ...
            #             originalspeciesindex = pickle.load(open(originalspeciesref + ".myindex", 'rb'))
                    ...
                    Utils.generateFasterRefIndex(pathTofn, os.path.join(options.compliantFasta.strip(),"."+fn.strip()) + ".myfasteridx",chrsignal="|")
                    duckrefindex = pickle.load(open(os.path.join(options.compliantFasta.strip(),"."+fn.strip()) + ".myfasteridx", 'rb'))                 
        ...
        for line in groupfile:
            ...
            for nameOf_s_p in linelist[1:]:
                ...
                RefSeqMap=Utils.getRefSeqBypos_faster(fileHmap[s_p.group(1)][0], fileHmap[s_p.group(1)][1], s_p.group(2), 1, 9999999999)
            ...
            print(Utils.encode_phyliplines(species_names, aaseqs,30), file=open(genefamilyalnname+".phy","w"))
        ...
        for fn in os.listdir(step2outpath):
            ...
            if os.path.isfile(pathTofn) and pathTofn.endswith("_align.fa.phy"):
                ...
                with open(pathTofn,'r') as pffffff:
                    physeqmap=Utils.decode_phyliplines(pffffff)
...
#                                             parallel_sites.append(i)
                ...
                if convergent_sites!={} or parallel_sites!={}:
                    ...
                    print("convergent sites:",file=finalfile);Utils.mapFormatPrint(convergent_sites, finalfile);print("parallel_sits:",file=finalfile);Utils.mapFormatPrint(parallel_sites,finalfile)
