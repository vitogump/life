...
parser = OptionParser()
parser.add_option("-h", "--homologousgene", dest="homologousgene",
...
MuscleInputFileName="muscleinseq.fa"
MuscleOutputFileName="muscleinseq.afa"
pamlInputCDSFileName="pamlinputfile.fa"
...
#load aa_cds_pair file and cdsfafile aafafile into memory
    for aa_cds_line in aa_cds_pair_file:
        ...
        if aafafileName!=None and cdsfafileName!=None:
            ...
            cdsindex={}
            try:
                aa_cds_filemap[speciesname][2] = pickle.load(open(aafafileName + ".myindex", 'rb'))
                aa_cds_filemap[speciesname][3] = pickle.load(open(cdsfafileName + ".myindex", 'rb'))
            except IOError:
                Util.generateIndexByChrom(aafafileName, aafafileName + ".myindex")
...
#make the fa file as the input of the file
    homotrscpttitle=re.split(r'\s+',homogenefile.readline())
    for homotrscptline in homogenefile:
        ...
        for trscpt in homotrscptlist:
            ...
            refSeqMap, currentChromNO, currentChromNO=Util.getRefSeqMap(aa_cds_filemap[curspecies][0],homotrscptlist[i])
            print(">"+homotrscptlist[i],file=muscleinfile)
            print("".join(refSeqMap[1:]),file=muscleinfile)
            i+=1
