...
def selectTAGsnp(filename):
    print("java -XX:-UseGCOverheadLimit  -Xmx120g  -jar ~/software/Haploview.jar -nogui -pedfile "+filename+".ped -info "+filename+".info -blockoutput GAB -log "+filename+".log -out "+options.outputfilename+filename+" -memory 120000 -maxDistance 300 -taglodcutoff 3 -tagrsqcutoff 0.6 -aggressiveTagging")
...
def splicevcf(curchr):
    ...
    while startpos<endpos:
        if False and not os.path.exists(re.search(r"^[^.]*",re.search(r"[^/]*$",options.vcffile).group(0)).group(0)+"chr"+curchr+"_"+str(i)+".ped"):
...
#         vcfrecOfcurchr=vcfobj.getVcfListByChrom(curchr,  MQfilter=0)
    ...
    mappedlist=[]
    pool1=Pool(int(len(vcfobj.chromOrder)))        
    pool1.map(splicevcf,vcfobj.chromOrder)
    pool1.close()
    pool1.join()
#     for curchr in vcfobj.chromOrder:

    #temp . this file has been TAGed as a test
    mappedlist.remove("filteredchr2_3")
