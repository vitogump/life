...
if __name__ == '__main__':
    ...
    for i in range(seed,seed+int(options.timestosample)):
        j+=1
        cmd=options.sampleTools+" -s"+str(i)+" "+options.p1fq+" "+str(options.sampletoreadscount)+" > "+options.sampleedfq1+str(j)
        print(cmd)
        if os.path.isfile(options.sampleedfq1+str(j)):
            print(options.sampleedfq1,"exist")
            continue
        a=os.system(cmd)
        if options.checkcounts:c=os.popen("wc -l "+options.sampleedfq1+str(j));print(options.sampleedfq1+str(j),"reads counts:",int(re.split(r'\s+',c.readline().strip())[0])/4)
        
        if a==0:print("seed ",i,"for "+options.sampleedfq1+" sample successfully!")
        if options.p2fq:
            cmd=options.sampleTools+" -s"+str(i)+" "+options.p2fq+" "+str(options.sampletoreadscount)+" > "+options.sampleedfq2+str(j)
            a=os.system(cmd)
            if options.checkcounts:c=os.popen("wc -l "+options.sampleedfq2+str(j));print(options.sampleedfq2+str(j),"reads counts:",int(re.split(r'\s+',c.readline().strip())[0])/4)
